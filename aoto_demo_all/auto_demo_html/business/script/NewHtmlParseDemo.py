...
from titanrun.common.NewHtmlBase import NewHtmlBase
...
class NewHtmlParse(NewHtmlBase):

    def new_htmlparse_pub(self, model="ride"):
        ...
        for i in xrange(self.api_len):
            ...
            for ini_name in self.ini_name_list:
                ...
                if api in default_data[0].keys() or api in default_data[4].keys() or api in default_data[5].keys():
                    break
            ...
            if ".db" not in api and ".dmldb" not in api:
                ...
                # sq特殊处理
                if interface_name in ["getVerifyCode.do", "Login.sso", "Ajax.sso"]:
                    handler_version_url = self.version_url.replace(r"/"+self.version_uri, "")
                    self.version_url = handler_version_url
                # # 同一流程里需要还原
                elif self.version_uri not in self.version_url:
                ...
                # 替换设置的参数值
                self.change_para_html(default_contant, self.data_source[i * 3 + 1 - j].split("||")[0])  # data_source[1]    data_source[3]...
